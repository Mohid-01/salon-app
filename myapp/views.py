...
def schedule_appointment(request):
    ...
    if not user_id or not salon_id or not appointment_date_str or not user_name or not user_email:
        return Response({'error': 'User ID, Salon ID, appointment date, user name, and user email are required'},
                        status=status.HTTP_400_BAD_REQUEST)

    try:
        # Get user and salon objects
        user = User.objects.get(id=user_id)
        salon = Salon.objects.get(id=salon_id)
    ...
    # Send email to the salon after successfully creating the appointment
    salon_email = salon.email  # Assuming the Salon model has an email field
    subject = f"New Appointment Scheduled by {user_name}"
    ...
    try:
        send_mail(
            subject,
            message,
            'yourapp@example.com',  # From email (make sure this is configured in your Django settings)
            [salon_email],  # To email (salon's email)
            fail_silently=False,
        )
    except Exception as e:
        return Response({'error': f'Failed to send email: {str(e)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

    return Response({'message': 'Appointment scheduled and email sent', 'appointment': serializer.data},
